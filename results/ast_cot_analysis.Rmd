...
This report presents the statistical analysis of **Run 2** of the experiment comparing **AST-Guided Chain-of-Thought (CoT) prompting** versus **Baseline prompting** for secure code generation. The study examines vulnerability counts across two large language models (Claude and GPT) using three detection tools (AST Parser, Bandit, and Semgrep).

**Research Design:** 2 × 2 Factorial Design

- **Independent Variable 1:** Model (Claude vs. GPT)
- **Independent Variable 2:** Prompt Type (Baseline vs. AST-Guided)
- **Dependent Variables:** Vulnerability count, Vulnerability density (per KLOC)
...
The histogram below shows the distribution of vulnerability counts across all samples.

```{r fig-distribution, fig.cap="Distribution of vulnerability counts across all samples. The majority of samples contain zero vulnerabilities, with a right-skewed tail."}
...
The boxplots below show the distribution of vulnerability counts across all four experimental conditions. Red diamonds indicate group means.

```{r fig-boxplot, fig.cap="Vulnerability count distributions by model and prompt type. Claude shows a notable reduction (18%) with AST-Guided prompting."}
par(mar = c(5, 5, 4, 2))
...
The bar chart below provides a clearer comparison of mean vulnerability counts with standard error bars.

```{r fig-bars, fig.cap="Mean vulnerability counts by condition with standard error bars. Error bars represent ±1 SE. Claude shows 18% reduction while GPT shows slight increase."}
par(mar = c(5, 5, 4, 2))
...
Three static analysis tools were used to detect vulnerabilities: a custom AST Parser, Bandit, and Semgrep.

```{r fig-tools, fig.cap="Total vulnerabilities detected by each tool across prompt conditions."}
par(mar = c(5, 5, 4, 2))
...
Before conducting parametric tests, we must verify that the data meets the required assumptions.
...
**Interpretation:** All conditions violate the normality assumption (p < .05). This is expected given the floor effect visible in the distribution. Non-parametric tests will be conducted alongside parametric tests for robustness.
...
**Interpretation:** Levene's test result indicates whether variances are homogeneous across groups.

---

# Analysis 1: Two-Way Factorial ANOVA

**Research Question:** Do Model type and Prompt type significantly affect vulnerability count, and is there an interaction effect?

**Design:** 2 (Model: Claude vs. GPT) × 2 (Prompt: Baseline vs. AST-Guided)

**Hypotheses:**

- H₁ₐ: Main effect of Model - Claude and GPT differ in vulnerability counts
- H₁ᵦ: Main effect of Prompt - AST-Guided and Baseline differ in vulnerability counts  
- H₁꜀: Interaction effect - The effect of prompt type depends on model type
...
The interaction plot below visualizes how the effect of prompt type differs between models.

```{r fig-interaction, fig.cap="Interaction plot showing Model x Prompt Type. Claude shows a steeper decline with AST-Guided prompting compared to GPT, suggesting differential effectiveness."}
par(mar = c(5, 5, 4, 2))
...
**Interpretation:** The interaction plot shows that Claude responds more strongly to AST-Guided prompting than GPT does.

---

# Analysis 2: Independent Samples t-test

**Research Question:** Does AST-Guided prompting significantly reduce vulnerability counts compared to Baseline prompting?
...
**Rationale:** Due to normality violations, the Mann-Whitney U test provides a robust non-parametric alternative.
...
**Research Question:** What are the relationships between different vulnerability detection tools and code characteristics?

```{r fig-correlation, fig.cap="Correlation heatmap showing relationships between vulnerability metrics. Darker red indicates stronger positive correlations."}
par(mar = c(6, 8, 4, 4))
...
**Research Question:** How well do Model, Prompt Type, and Lines of Code predict total vulnerability count?
...
The full regression model was statistically significant, explaining `r round(reg_summary$r.squared * 100, 1)`% of variance in vulnerability counts. Key findings:

- **Lines of Code (LOC):** Significant predictor (p < .001). Each additional line of code increases expected vulnerabilities by ~0.014.
- **Model (GPT vs Claude):** Significant (p = .003). GPT produced approximately 0.60 more vulnerabilities per sample than Claude on average.
- **Prompt Type (AST-Guided vs Baseline):** Not significant (p = .606). AST-guided prompting did not significantly predict vulnerability count when controlling for other factors.
- **Interaction (Model × Prompt Type):** Not significant (p = .568). The effect of prompt type did not differ significantly between models in this analysis.

---

# Vulnerability Density Analysis

Vulnerability density normalizes vulnerability counts by lines of code, providing a fair comparison across samples of different lengths.
...
```{r fig-reduction, fig.cap="Percent reduction in vulnerability count achieved by AST-Guided prompting compared to Baseline."}
par(mar = c(5, 8, 4, 2))
...
4. **Detection Tool Agreement:** All three tools show positive intercorrelations, supporting the validity of the multi-tool approach


*Report generated using R version `r R.version.string`*
